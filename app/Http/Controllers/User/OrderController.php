    ...
    public function statusChange(Request $request)
    {
        ...
        if($request->status == 6){
            ...
            $emailData = [
                ...
                'voucher_number' =>$order->document_number,
                ];
            ...
            //shipment address management for mail
            $address = $order->custom_postal_code.' '.$order->prefecture.' '.$order->custom_municipility.' '.$order->custom_address.' '.$order->custom_room_no;
            ...
            $emailData = [
                ...
                'product_name'  =>$product->title,
                'point' =>$OrderDetail->total_point,
                'user_name' =>$order->ShipmentFirstName.' '.$order->ShipmentSecondName,
            ...
            //send mail to buyer
            $order = Order::find($OrderDetail->order_id);
            $buyer = User::with('profile')->where('id', $order->user_id)->first();
            $product = Product::find($OrderDetail->product_id);
            ...
            $emailData = [
                'name' =>  $buyer->first_name.' '.$buyer ->last_name ,
                'register_token' =>'',
                'subject' => '【Crofun】商品は発送準備中です',
                'from_email' => 'noreply@crofun.jp',
                'from_name' => 'Crofun',
                'template' => 'user.email.36',
                'root'     => $request->root(),
                'email'     => $buyer->email,
                'product_name'  =>$product->title,
                'point' =>$OrderDetail->total_point,
