...
class AdminUserController extends Controller
{
    ...
    public function data()
    {
        ...
        ->addColumn('action', function ($result) {
            $output = '';
            if ($result->status==0) {
                $output .= '<a href="'.route('admin-admin-user-status-change', ['package_id' => $result->id, 'status'=> 1]).'" class="btn btn-sm btn-success inline">有効にする</a> ';
            }
            else{
                $output .= '<a href="'.route('admin-admin-user-status-change', ['package_id' => $result->id, 'status'=> 0]).'" class="btn btn-sm btn-danger inline">無効にする </a> ';
            }
            // $output .= '<a href="'.route('admin-admin-user-edit', ['package_id' => $result->id]).'" class="btn btn-sm btn-info inline">編集</a> ';
                    // <a href="'.route('admin-admin-user-delete', ['package_id' => $result->id]).'" class="btn btn-sm btn-danger delete-sure inline">削除する</a>';
            return $output;
