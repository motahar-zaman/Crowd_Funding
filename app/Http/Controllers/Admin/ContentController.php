...
class ContentController extends Controller
{
    ...
    public function data(Request $request)
    {
        ...
        ->addColumn('action', function ($result) {
            $output = '';
            if ($result->status==0) {
                $output .= '<a href="'.route('admin-content-status-change', ['id' => $result->id, 'status'=> 1]).'" class="btn btn-sm btn-success">公開にする</a> ';
            }
            else{
                $output .= '<a href="'.route('admin-content-status-change', ['id' => $result->id, 'status'=> 0]).'" class="btn btn-sm btn-danger">非公開にする</a> ';
            }
            $output .= '<a href="'.route('admin-content-edit', ['id' => $result->id]).'" class="btn btn-sm btn-info">編集</a>';
