			    ...
			    <div class="mt20">
			        ...
			        <section>
						  ...
						  <div class="form-group">
						    ...
						    <input type="file" class="form-control required product_image" placeholder="" name="image" accept=".jpg,.png,.jpeg">
							<div class="">
						  ...
						  <div class="form-group">
						    ...
						    <input type="file" class="form-control company_info_image" name="company_info_image" accept=".jpg,.png,.jpeg">
							<div class="">
								<span style="font-size:12px;">*画像は 2MB 以下である必要があります。</span>
							</div>
						  </div>
		...
		form.children("div").steps({
		    ...
		    onStepChanged: function (event, currentIndex, newIndex)
		    {
		        if(currentIndex == 1){
					...
					var product_details = []
					$('.p_details').val().split('\n').map(function(item){
						product_details.push('<span>'+item+'</span>'+'<br/>')
					})
					$('.product_details').html(product_details);

					$('.product_company_name').html($('.company_name').val());

					var product_company_info_details = []
					$('.p_company_info').val().split('\n').map(function(item){
						product_company_info_details.push('<span>'+item+'</span>'+'<br/>')
					})
					$('.product_company_info').html(product_company_info_details);
					
		        }
		    },
		    onFinishing: function (event, currentIndex)
		    {
				$('#product-submit-message').modal('show');
		        form.validate().settings.ignore = ":disabled,:hidden";
        		return form.valid();
        		// return true;
		    },
		    onFinished: function (event, currentIndex)
		    {
		        $(document).ready(function(){
					$('#submit_button').on('click', function(e){
						...
						// $("$submit_button").attr("disabled", true).delay(2000).attr("disabled", false);
						form.submit()
					});
    			});
		    }
