							...
							@if($product <= 0)
								<ul class="list-inline project_category_data pt-4">
									<li class="list-inline-item">TOP ＞
										<a style="color:#292b2c" href="{{route('front-product-list')}}">カタログ一覧</a>
										＞
										<a style="color:#292b2c" href="{{route('front-product-list', ['c' => $product->subCategory->category->id])}}">
											{{ $product->subCategory->category->name }}
										</a>
										＞
										<a style="color:#292b2c" href="{{route('front-product-details', ['id' => $product->id])}}">
											{{ $product->title }}
										</a>
									</li>
								</ul>
