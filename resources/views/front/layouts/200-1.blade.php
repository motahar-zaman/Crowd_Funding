														...
														@else
															@if($project->end < date("Y-m-d", strtotime('now')))
																<div class=" mb-4 mt-1 col-md-12">
																	<div class=" text-white btn btn-md reward_button btn-block" name="button" style="background:#cccccc;">このリターンを選択する</div>
																</div>
															@else
																@if($project->user_id == $user_profile->id)
																	<div class=" mb-4 mt-1 col-md-12">
																		<div class="noHover text-white btn btn-md btn-block" name="button" style="background:#0099ff;;background-color:#cccccc;">このリターンを選択する</div>
																	</div>
																@else
																	<div class=" mb-4 mt-1 col-md-12">
																		<a href="/user/invest/{{ $project->id }}?reward={{ $reward->id }}" class="reward_button text-white btn btn-md btn-block <?php echo $project->start > date("Y-m-d 00:00:00")?'disabled':'enabled';?>" name="button" style="background:#0099ff;">このリターンを選択する</a>
																	</div>
																@endif
															@endif
														@endif
