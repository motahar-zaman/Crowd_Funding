...
<p style="margin-top:20px;font-size: 16px;" class="sidebar_font">
...
<ul class="product_list_link" style="font-family: w3 !important;">
	...
	<li>
		<a style="color: black; font-size:14px;text-decoration: none;" href="{{route('front-product-list', ['c' => $pc->id])}}">
			{{$pc->name}}
		</a>
		...
		<?php foreach($pc->subCategory as $sc){?>
			<li>
				<a style="color: black; font-size:14px;text-decoration:none" href="{{route('front-product-list', ['sc' => $sc->id])}}">
					{{$sc->name}}
